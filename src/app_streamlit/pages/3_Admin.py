...
with c1:
    ...
    if st.button("Refresh"):
        info = ctx.vectordb.table_info(table_name)
        st.write(
            {
                "db_dir": str(info.get("db_dir")),
                "table": info.get("table_name"),
                "vector_col": info.get("vector_col"),
...
with c2:
    ...
    if st.button("Check duplicates by image_id"):
        rows = ctx.vectordb.sample_rows(table_name=table_name, limit=300000, columns=["image_id"])
...
with c3:
    ...
    if st.button("Delete", type="secondary", disabled=not confirm):
        res = ctx.vectordb.delete_where(table_name=table_name, where=delete_where)
        st.success(res)
